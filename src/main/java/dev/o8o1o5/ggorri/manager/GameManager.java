...
public class GameManager {
    ...
    private static final long DAMAGE_TRACK_DURATION_MILLIS = 15 * 1000; // 15초
    ...
    public boolean stopGame() {
        ...
        for (UUID uuid : new ArrayList<>(playersInGame.keySet())) { // ConcurrentModificationException 방지
            ...
            if (player != null) {
                resetPlayer(player); // 인벤토리 초기화, 기본 장비 제거 등
                if (gameWorld != null) {
    ...
    private void resetPlayer(Player player) {
        ...
        player.setAllowFlight(false); // 날기 허용 초기화
        player.setFlying(false); // 비행 중지
        plugin.getLogger().log(Level.FINE, "[GGORRI] " + player.getName() + "님의 플레이어 상태가 초기화되었습니다.");
    }
    ...
    public void recordPlayerDamage(UUID damagerUUID, UUID victimUUID) {
        ...
        plugin.getLogger().log(Level.FINE, "[GGORRI Debug] 데미지 기록: " + plugin.getServer().getPlayer(damagerUUID).getName() + " -> " + plugin.getServer().getPlayer(victimUUID).getName());
    }
    ...
    public UUID getLastAttacker(UUID victimUUID) {
        ...
        if (lastAttacker != null) {
            plugin.getLogger().log(Level.FINE, "[GGORRI Debug] 가장 최근 공격자: " + plugin.getServer().getPlayer(lastAttacker).getName() + " -> " + plugin.getServer().getPlayer(victimUUID).getName());
        }
        return lastAttacker;
    }
    ...
    public void clearDamageRecordsForPlayer(UUID playerUUID) {
        ...
        plugin.getLogger().log(Level.FINE, "[GGORRI Debug] 플레이어 " + plugin.getServer().getPlayer(playerUUID).getName() + "의 데미지 기록 정리 완료.");
    }
    ...
    public void endGame(UUID winnerLeaderUUID) {
        ...
        for (UUID uuid : new ArrayList<>(playersInGame.keySet())) {
            Player player = plugin.getServer().getPlayer(uuid);
            if (player != null) {
                resetPlayer(player); // 인벤토리, 체력, 게임모드 등 초기화
    ...
    /**
     * Return game data of specific player
     */
